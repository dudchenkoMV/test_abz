...
/**
 * blank HTML for cursor positions
...
/**
 * padding blankHTML if node is empty (for cursor positions)
 */
...
/* harmony default export */ const range = ({
  ...
  /**
   * create WrappedRange from node after positions
   *
   * @param {Node} node
   * @return {WrappedRange}
   */
...
var TableResultAction = function TableResultAction(startPoint, where, action, domTable) {
  ...
  /**
   * Define virtual table positions info object.
   *
   * @param {int} rowIndex Index positions in line of virtual table.
   * @param {int} cellIndex Index positions in column of virtual table.
   * @param {object} baseRow Row affected by this positions.
   * @param {object} baseCell Cell affected by this positions.
   ...
   *
   * @param {object} virtualTableCellObj Object of specific positions on virtual table.
    ...
    */
    ...
    value: function fontStyling(target, value) {
      ...
      if (rng !== '') {
        ...
        //  - also bogus character needed for cursor positions
    ...
    value: function replace() {
      ...
      if ($item.length) {
        ...
        if (this.matchingWord !== null && this.matchingWord.length === 0) {
          this.lastWordRange.so = this.lastWordRange.eo; // Else si > 0 and normal case -> adjust range "before" for correct positions of insertion
      ...
      if (!lists.contains([key.code.ENTER, key.code.UP, key.code.DOWN], e.keyCode)) {
        ...
        if (this.hints.length && keyword) {
          ...
          if (bnd) {
            ...
            this.$content.find('.note-hint-item:first').addClass('active'); // set positions for popover after group is created
