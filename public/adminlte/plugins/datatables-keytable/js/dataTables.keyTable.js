...
$.extend( KeyTable.prototype, {
	...
	_focus: function ( row, column, shift, originalEvent )
	{
		...
		// in its displayed positions
		if ( pageInfo.serverSide ) {
			row -= pageInfo.start;
		}

		// Get the cell from the current positions - ignoring any cells which might
	 ...
	 * @param  {string} posOff    `positions` or `offset` - which to use for the
	 *   calculation. `offset` for the document, otherwise `positions`
	 * @private
	 */
	_scroll: function ( container, scroller, cell, posOff )
	{
		...
		// If Scroller is being used, the table can be `positions: absolute` and that
	...
	_shift: function ( e, direction, keyBlurable )
	{
		...
		// row's current positions by the display start point
		if ( pageInfo.serverSide ) {
